...
#define PROJECTION  7
#define ORTHOGONAL  8
#define PERSPECTIVE 9
#define FuckProject 10
...
{
	Vector3 forward(
		main_camera.center[0] - main_camera.position[0],
		main_camera.center[1] - main_camera.position[1],
		main_camera.center[2] - main_camera.position[2]
	);
	Vector3 rz = -forward.normalize();
	/* rx points to the right */
	Vector3 rx = forward.cross(main_camera.up_vector);
	rx.normalize();
	/* ry points to the upwards */
	...
	view_matrix = rotation * transform;
...
void setOrthogonal()
{
	...
	project_matrix = Matrix4(
		2 / (proj.right - proj.left), 0, 0, -(proj.right + proj.left) / (proj.left - proj.right),
...
void onDisplay(void)
{
	...
	/* Change PERSPECTIVE or ORTHOGONAL*/
	if (ProjectionMode == ORTHOGONAL) setOrthogonal();
	else setPerspective();
...
void instruction() {
	printf("\n======Instruction======\n\n");
	printf("Geometrical transformation :\n\n") ;
	printf("* Key t : translation\n* Key r : rotation\n* Key s : scaling\n\n" );
	printf("Viewing transformation     :\n\n");
	printf("* Key c : center\n* Key e : eye\n* Key u : upVector\n\n");

	printf("Change Projection Matrix   :\n\n"); 
	printf("* Key o : orthogonal\n* Key p : perspective\n");
	printf("\nChange avatar model by\n* Key z / x\n\n");
	printf("Print all matrix value by\n* Key i\n\n");
	printf("Wire mode by\n* Key w\n\n");
	printf("=======================\n");
}
...
void onKeyboard(unsigned char key, int x, int y)
{
	...
	case 'o':
		...
		printf("\nya change to ORTHOGONAL\n");
		break;
	case 'p':
		ProjectionMode = PERSPECTIVE;
		printf("\nya change to PERSPECTIVE\n");
		break;
	case 'e':
		mode = EYE;
		break;
	...
	case 'i':
		/*
		 * I 鍵要印出所有參數值 :
		 * 此model的position、rotation、scaling值，
		 * 相機之position、center，projection之矩陣模式
		 * 以及其設定之left、right、up、bottom、nearClip、farClip、fov值
		 * 不用顯示矩陣值
		 */
		printf("\n**********************************************************\n\n");
		cout << "Model postion    : " << models[cur_idx].position   << endl;
		cout << "Model rotation   : " << models[cur_idx].rotation   << endl;
		cout << "Model scaling    : " << models[cur_idx].scale      << endl;
		cout << "Camera position  : " << main_camera.position       << endl;
		cout << "Camera center    : " << main_camera.center         << endl;
		cout << "Camera up vector : " << main_camera.up_vector      << endl;
		...
		cout << "fov              : " << proj.fovy                  << endl;
		printf("Projection Mode   : ");
		if (ProjectionMode == ORTHOGONAL) printf("======Orthogonal projection=======\n");
		else printf("=====Perspective projection======\n"); // ProjectionMode == PERSPECTIVE
		printf("\n**********************************************************\n");
		break;
	case 't':
		mode = TRANSLATE;
